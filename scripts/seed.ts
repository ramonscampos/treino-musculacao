...
import { ANDRESSA_DATA, parseRest, parseSets, RAMON_DATA } from "./seed-data";
...
async function seed() {
	...
	await db.execute("DELETE FROM plan_exercises");
	await db.execute("DELETE FROM workout_plans");
	await db.execute("DELETE FROM users");
	try {
		await db.execute(
			"DELETE FROM sqlite_sequence WHERE name IN ('workout_plans', 'plan_exercises', 'users')",
		);
	} catch {
		// sqlite_sequence might not exist if tables haven't used auto-increment yet
	}

	// Ensure users have a unique index on name to prevent future duplicates
	await db.execute("CREATE UNIQUE INDEX IF NOT EXISTS ux_users_name ON users (name)");
	...
	const userMap = Object.fromEntries(
		userRows.map((r) => [r.name as string, r.id as number]),
	);
	...
	for (const d of [RAMON_DATA, ANDRESSA_DATA]) {
		for (const day of Object.values(d)) {
			for (const ex of day.exercises) {
				if (!exDescriptions.has(ex.name))
					exDescriptions.set(ex.name, ex.description);
			}
		}
	}
	...
	async function seedUser(userName: string, data: typeof RAMON_DATA) {
		...
		for (const [dayKey, dayData] of Object.entries(data)) {
			const result = await db.execute({
				...
				args: [
					userId,
					dayData.title.split("—")[1]?.trim() ?? dayKey,
					dayKey,
					dayData.title,
				],
			});
